...
butil::Mutex g_map_lock;
typedef butil::FlatMap<void*, ibv_mr*> Map;
static Map* g_addr_map = NULL;
...
static void GlobalRelease() {
    {
        BAIDU_SCOPED_LOCK(g_map_lock);
        if (g_addr_map) {
            for (Map::iterator it = g_addr_map->begin();
...
static void GlobalRdmaInitializeOrDieImpl() {
    ...
    g_addr_map = new (std::nothrow) Map;
    if (!g_addr_map) {
