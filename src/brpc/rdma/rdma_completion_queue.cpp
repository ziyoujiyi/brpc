...
int RdmaCompletionQueue::Init(int cpu) {
    ...
    ibv_cq* cq = ibv_create_cq(ctx, _cq_size, NULL, cq_channel, _cpu_index);
    if (!cq) {
        return -1;
    }
    _cq = cq;
...
inline int RdmaCompletionQueue::Refresh() {
    CleanUp();
    return Init(_cpu_index);
}
