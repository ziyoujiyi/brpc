...
#ifndef BRPC_RDMA
    if (BAIDU_UNLIKELY(s->_rdma_state == RDMA_UNKNOWN)) {
#else
...
ssize_t Socket::DoRead(size_t size_hint) {
    ...
    if (ssl_state() == SSL_OFF) {
#ifndef BRPC_RDMA
        if (BAIDU_UNLIKELY(_rdma_state == RDMA_UNKNOWN)) {
#else
...
void Socket::DebugSocket(std::ostream& os, SocketId id) {
    ...
    const int preferred_index = ptr->_preferred_index;
...
inline SocketPool::SocketPool(const SocketOptions& opt)
    : _options(opt), _remote_side(opt.remote_side), _count(0) {
}
...
inline void SocketPool::ReturnSocket(Socket* sock) {
    ...
    if (_count.fetch_add(1, butil::memory_order_relaxed) <
